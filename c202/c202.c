...
//provede inicializaci zásobníku... Vrchol zásobníku se nastaví na hodnotu -1. To znamená, že vrchol není na prvním indexu, tím pádem je prázdný
    if(s == NULL){
...
//Pokud je vrchol zásobníku nastaven na -1, znamená, že je prázdný. viz stackInit()
//funkce vrátí nenulovou hodnotu (True), pokud je prázdný a nenulovou (False), pokud není
    return (s->top == -1);
}
...
//Pokud je vrchol zásobníku nastaven na poslední index pole, znamená to, že je zásobník plný
//např... velikost zásobníku je 8... indexy od 0 do 7. pokud se poslední index + 1 rovná velikosti, je zásobník plný
// 7 + 1 == 8
//funkce vrací nenulovou hodnotu (True), pokud je zásobník plný a nulovou (False), pokud plný není
...
//vrací hodnotu z vrcholu zásobníku pomocí pointeru
    if(stackEmpty(s)){
        stackError(SERR_TOP);
    }
    else{
...
//Pop znamená, že se ze zásobníku odebírá
//dekrementuje se hodnota ukazujicí na vrchol zásobníku
    if(!stackEmpty(s)){
        s->top--;
    }
}
...
//Push znamená, že se do zásobníku přidává
//incrementuje se hodnota ukazujicí na vrchol zásobníku (index v poli, kde je poslední hodnota)
//poté se na tento index přidá pushovaná hodnota
